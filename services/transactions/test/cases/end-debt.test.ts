...
const step1 = (context: Context) => ({
  ...
  when('I set JSON request body as:', (body: string) => {
    ...
    bus.addHandler(
      END_DEBT_COMMAND,
      END_DEBT_COMMAND_HANDLER(context.container),
    );
  ...
  when(/^I request PUT method at "([^"]*)" url$/, async (url: string) => {
    context.res = await supertest(
      express().put(url, express.json(), endDebtRouter(context.container)),
    )
      .put(url)
...
defineFeature(loadFeature('./test/features/end-debt.feature'), test => {
  ...
  test('Should complete a debt request', step1(context));
