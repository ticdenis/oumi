...
import {
  ...
  denyDebtRequestRouter,
} from '../../src/cases/deny-debt-request';
import {
  CommandBus,
  Debt,
...
const step1 = (context: Context) => ({
  ...
  when('I set JSON request body as:', (body: string) => {
    ...
    commandRepository.denyDebtRequest(Arg.any()).returns(Promise.resolve());
    ...
    bus.addHandler(
      DENY_DEBT_REQUEST_COMMAND,
      DENY_DEBT_REQUEST_COMMAND_HANDLER(context.container),
    );
  ...
  when(/^I request PUT method at "([^"]*)" url$/, async (url: string) => {
    context.res = await supertest(
      express().put(
        url,
        express.json(),
        denyDebtRequestRouter(context.container),
      ),
    )
      .put(url)
...
defineFeature(
  loadFeature('./test/features/deny-debt-request.feature'),
  test => {
    ...
    test('Should deny a debt request', step1(context));
