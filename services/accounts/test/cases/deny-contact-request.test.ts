...
import {
  ...
  denyContactRequestRouter,
} from '../../src/cases/deny-contact-request';
import {
  CommandBus,
  Contact,
...
const step1 = (context: Context) => ({
  ...
  when('I set JSON request body as:', (body: string) => {
    ...
    commandRepository.denyRequest(Arg.any()).returns(Promise.resolve());
  ...
  when(/^I request PUT method at "([^"]*)" url$/, async (url: string) => {
    context.res = await supertest(
      express().put(
        url,
        express.json(),
        denyContactRequestRouter(context.container),
      ),
    )
      .put(url)
...
defineFeature(
  loadFeature('./test/features/deny-contact-request.feature'),
  test => {
    ...
    test('Should deny a contact request', step1(context));
