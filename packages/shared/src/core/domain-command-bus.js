...
class DomainCommandBus {
    ...
    async dispatch(command) {
        if ('command' !== command.type) {
            return Promise.reject(new _1.CommandDomainError('INVALID_COMMAND_TYPE', `Argument received not is a command is a <${command.type}>.`));
        }
        if (!this._commandHandlers.has(command.name)) {
            return Promise.reject(new _1.CommandDomainError('COMMAND_NOT_FOUND', `Command Handler for <${command.name}> not found.`));
        }
